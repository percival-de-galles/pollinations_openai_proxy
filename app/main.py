...
app = FastAPI(title="Pollinations OpenAI-Compatible Proxy", version="0.1.0")
...
async def list_models(request: Request, authorization: Optional[str] = Header(None)) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Text models
        try:
            t = await fetch_json(client, f"{UPSTREAM_TEXT_BASE}/models")
            if isinstance(t, list):
                models_text = [
                    {"id": m, "object": "model"} if isinstance(m, str) else m for m in t
                ]
        except Exception:
            pass
        # Image models
        try:
            i = await fetch_json(client, f"{UPSTREAM_IMAGE_BASE}/models")
            if isinstance(i, list):
                models_image = [
                    {"id": m, "object": "model"} if isinstance(m, str) else m for m in i
                ]
        except Exception:
            pass
...
async def chat_completions(request: Request, body: Dict[str, Any], authorization: Optional[str] = Header(None)) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            r = await client.post(
                f"{UPSTREAM_TEXT_BASE}/openai", json=body, timeout=120
            )
            r.raise_for_status()
            data = r.json()
        ...
        try:
            r2 = await client.get(f"{UPSTREAM_TEXT_BASE}/{quote(prompt)}", timeout=120)
            r2.raise_for_status()
            text = r2.text
...
async def completions(request: Request, body: Dict[str, Any], authorization: Optional[str] = Header(None)) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        try:
            r = await client.get(f"{UPSTREAM_TEXT_BASE}/{quote(prompt)}", timeout=120)
            r.raise_for_status()
            text = r.text
        except httpx.HTTPError as e:
            raise HTTPException(status_code=502, detail=f"Upstream error: {e}")
